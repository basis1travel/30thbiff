...
try:
    ...
    with tab4:
        ...
        if df_2024.empty or '상호' not in df_2024.columns:
            ...
            map_data = df_2024_filtered[df_2024_filtered['주소'].notna() & (df_2024_filtered['주소'].str.strip() != '')].copy()
            if not map_data.empty:
                with st.spinner("주소를 좌표로 변환하는 중... (시간이 걸릴 수 있습니다)"):
                    map_data[['lat', 'lon']] = map_data['주소'].apply(lambda x: pd.Series(geocode_address(x)))
